...
> ⚠️ **采样率处理**：VAD 和 ASR 模型固定要求 16kHz 输入。客户端指定的 `sample_rate` 无论为何值，服务器内部均会重采样至 16000 Hz 后处理。建议客户端直接发送 16kHz 音频以避免额外的重采样开销。
...
A: 检查音频格式：
1. 必须是 PCM 16-bit，单声道，小端序
2. 确认已通过 URL 参数 `?sample_rate=16000` 正确指定采样率
3. 服务器内部会自动重采样至 16000 Hz，但建议直接发送 16kHz 音频以获得最佳质量
